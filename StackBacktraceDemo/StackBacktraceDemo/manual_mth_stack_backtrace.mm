...
#include "manual_mth_stack_backtrace.h"
...
typedef struct _manual_mth_stackframe_entity {
    const struct _manual_mth_stackframe_entity *const previous;
    const uintptr_t return_address;
} manual_mth_stackframe_entity;

static kern_return_t manual_mth_mach_copy_mem(const void *const src, void *const dst, const size_t num_bytes) {
...
manual_mth_stack_backtrace *manual_mth_malloc_stack_backtrace() {
    manual_mth_stack_backtrace *stackframes = (manual_mth_stack_backtrace *)malloc(sizeof(manual_mth_stack_backtrace));
    if (stackframes) {
        memset(stackframes, 0, sizeof(manual_mth_stack_backtrace));
    }
    return stackframes;
}

void manual_mth_free_stack_backtrace(manual_mth_stack_backtrace *stackframes) {
    if (stackframes == NULL)
        return;
...
bool manual_mth_stack_backtrace_of_thread(thread_t thread, manual_mth_stack_backtrace *out_stack_backtrace, const size_t backtrace_depth_max, uintptr_t top_frames_to_skip) {
    ...
    // get frame point
    manual_mth_stackframe_entity frame = {NULL, 0};
    const uintptr_t frame_ptr = machine_context.__ss.MT_FRAME_POINTER;
    if (frame_ptr == 0 || manual_mth_mach_copy_mem((void *)frame_ptr, &frame, sizeof(frame)) != KERN_SUCCESS) {
        out_stack_backtrace->frames_size = frames_size;
        return false;
    }
    ...
    for (; frames_size < backtrace_depth_max; frames_size++) {
        ...
        if (backtrace_frames[frames_size] == 0 || frame.previous == 0 || manual_mth_mach_copy_mem(frame.previous, &frame, sizeof(frame)) != KERN_SUCCESS) {
            break;
        }
    }
